...
function _compileIndex( name, options ){
  ...
  _.each( options.articles.items, function( article, index, arr ){
    ...
    if( ( index + 1 ) % pageSize === 0 || index === 0 ){
      ...
      options.articles.pageSize = pageSize;//options.articles.items.splice( index, 5 );
      options.articles.pageNum = pageNum;
      options.articles.pageLength = Math.ceil( arr.length / pageSize );
...
function _compilePages( name, options ){
  var template = _loadTemplate( 'myBlog', 'page' );
